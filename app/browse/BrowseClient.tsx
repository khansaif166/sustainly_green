    ...
    <>
      ...
      <main className="max-w-full mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-[var(--color-bg-white)] border border-[var(--color-border)] rounded-2xl p-4 grid md:grid-cols-4 gap-4">
          <select value={type} onChange={(e) => updateFilter("type", e.target.value)} className="input">
        ...
        {/* RESULTS */}
        {loading ? (
          <p className="text-sm text-[var(--color-text-muted)] text-center">Loading listings…</p>
        ) : products.length === 0 ? (
          <p className="text-sm text-[var(--color-text-muted)] text-center">No listings found.</p>
        ) : (
            ...
            {products.map((p) => (
              ...
              >
                <div className="h-60 bg-[var(--color-bg-soft)] rounded-xl mb-3 overflow-hidden">
                  {p.images?.[0] && (
                    <img src={p.images[0]} className="h-full w-full object-cover" alt={p.title} />
                  )}
                </div>
